...
class RankListwiseOSLLM(RankLLM):
    ...
    def create_prompt(
        ...
        while True:
            if max_length < 150:
                print(
                    f"Max length too low, reducing query length to length {max_length}"
                )
                query = " ".join(query.split()[: int(max_length)])
            conv = (
            ...
            conv.append({"role": "user", "content": fix_text(input_context)})
